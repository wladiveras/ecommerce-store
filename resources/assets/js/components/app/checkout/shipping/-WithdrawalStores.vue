<template>
  <div class="withdrawal-locations">
    <div class="d-flex flex-wrap row">
      <b-form-radio-group v-model="chosen" class="hide-mark">
        <template v-for="location in locations">
          <b-form-radio :key="location.id" :value="location.id" class="mb-3">
            <div class="shipping-item item rounded p-2 border" @click="togglePayment(location)">
              <div class="flex-wrap d-flex justify-content-between">
                <span class="f-15 font-weight-bold text-uppercase">{{location.name}}</span>
                <check-circle />
              </div>
              ...
              <div class="f-12 f-space">
                {{location.address.street}}, {{location.address.number}} - {{location.address.district}} - {{location.address.zip_code}} - {{location.address.state}} - {{location.address.city}}
              </div>
            </div>
          </b-form-radio>
        </template>
